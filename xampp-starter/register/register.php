...
if(isset($_POST['submit'])){
    ...
    $student = $_POST['isStudent'] ?? false;
    $professor = $_POST['isProf'] ?? false;
    $ta = $_POST['isTA'] ?? false;
    $admin = $_POST['isAdmin'] ?? false;
    $courses = $_POST['courses'] ?? false;
    $time = date('d-m-y h:i:s');

    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)
         && (null !== ($student || $professor || $ta || $admin)) && !empty($courses))
		{
